...
syn keyword sqlKeyword CALL CASCADE CASCADED CASE CAST CATALOG CEILING CHAR CHAR CHAR_LENGTH CHARACTER CHARACTER_LENGTH CHARINDEX CHECK CHECKPOINT CLASS CLOB CLOSE CLUSTERED COALESCE COL_LENGTH COL_NAME COLLATE COLLATION COLLATIONPROPERTY COLUMN COLUMNPROPERTY COMMIT COMPLETION COMPUTE CONCAT_NULL_YIELDS_NULL CONNECT CONNECTION CONSTRAINT CONSTRAINTS CONSTRUCTOR CONTAINSTABLE CONTINUE CONVERT CORRESPONDING COS COT COUNT CREATE CROSS CUBE CURRENT CURRENT_DATE CURRENT_PATH CURRENT_ROLE CURRENT_TIME CURRENT_TIMESTAMP CURRENT_USER CURSOR CURSOR_CLOSE_ON_COMMIT CURSOR_STATUS CYCLE CATCH
...
syn keyword sqlKeyword MAP MATCH MAX MIN MINUTE MODIFIES MODIFY MODULE MONEY MONTH MERGE
syn keyword sqlKeyword NAMES NATIONAL NATURAL NCHAR NCHAR NCLOB NEW NEWID NEXT NO NOCHECK NOCOUNT NOEXEC NONCLUSTERED NONE NOT NTEXT NULL NULLIF NUMERIC NUMERIC_ROUNDABORT NVARCHAR NOLOCK
...
syn keyword sqlKeyword QUERY QUERY_GOVERNOR_COST_LIMIT QUOTED_IDENTIFIER QUOTENAME RADIANS RAISERROR RAND READ READS READTEXT REAL RECONFIGURE RECURSIVE REF REFERENCES REFERENCING RELATIVE REMOTE_PROC_TRANSACTIONS REPLACE REPLICATE REPLICATION RESTORE RESTRICT RESULT RETURN RETURNS REVERSE REVOKE RIGHT ROLE ROLLBACK ROLLUP ROUND ROUTINE ROW ROWCOUNT ROWCOUNT_BIG ROWGUIDCOL ROWS RTRIM RULE READUNCOMMITTED RAISE
...
syn keyword sqlKeyword TAN TEMPORARY TERMINATE TEXT TEXTPTR TEXTSIZE TEXTVALID THAN THEN TIME TIMESTAMP TIMEZONE_HOUR TIMEZONE_MINUTE TINYINT TO TOP TRAILING TRAN TRANSACTION TRANSLATE TRANSLATION TREAT TRIGGER TRIM TRUE TRUNCATE TSEQUAL TYPE_PROPERTY TRY
...
syn keyword sqlKeyword XACT_ABORT XML YEAR ZONE
...
syn keyword sqlFunction ERROR_NUMBER ERROR_MESSAGE
...
syn match sqlType "\<[a-zA-Z]\w\+_Type\>"
syn match sqlIdentifier "\(@\|#\)[a-zA-Z_]\+\>"
syn match sqlCommvault ":\w\+"

syn match sqlStoredProc "dbo\.\w\+"
...
if version >= 508 || !exists("did_sql_syn_inits")
	...
	HiLink sqlStoredProc        Function
    HiLink sqlOption	        PreProc
    HiLink sqlIdentifier        Special
	HiLink sqlCommvault        PreProc
